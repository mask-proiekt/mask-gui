...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting maskd " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM maskd.exe");
#else
                QProcess::execute("pkill maskd");
#endif
            }
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/maskd.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/maskd";
#endif
