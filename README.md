...
This is the GUI for the [core Mask implementation](https://github.com/mask-project/mask). It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of Mask that uses the protocol and network in a compatible manner.
...
## Installing the Mask GUI from a package

Packages are available for

* Arch Linux via AUR: [mask-wallet-qt](https://aur.archlinux.org/packages/mask-wallet-qt/)
* Void Linux: xbps-install -S mask-core
* GuixSD: guix package -i mask-core

Packaging for your favorite distribution would be a welcome contribution!

## Compiling the Mask GUI from source

### On Linux:

(Tested on Ubuntu 17.10 x64, Ubuntu 18.04 x64 and Gentoo x64)

1. Install Mask dependencies
...
3. Clone repository
    ...
    `git clone https://github.com/mask-project/mask-gui-wallet.git`
...
3. Install [mask](https://github.com/mask-project/mask) dependencies:
...
6. Grab an up-to-date copy of the mask-gui-wallet repository
...
The Mask GUI on Windows is 64 bits only; 32-bit Windows GUI builds are not officially supported anymore.

1. Install [MSYS2](https://www.msys2.org/), follow the instructions on that page on how to update system and packages to the latest versions

2. Open an 64-bit MSYS2 shell: Use the *MSYS2 MinGW 64-bit* shortcut, or use the `msys2_shell.cmd` batch file with a `-mingw64` parameter

3. Install MSYS2 packages for Mask dependencies; the needed 64-bit packages have `x86_64` in their names

4. Do not run "pacman -Syuu" on MSYS2. The update will break the build.

    ```
    pacman -S mingw-w64-x86_64-toolchain make mingw-w64-x86_64-cmake mingw-w64-x86_64-boost mingw-w64-x86_64-openssl mingw-w64-x86_64-zeromq mingw-w64-x86_64-libsodium mingw-w64-x86_64-hidapi
    ```

    You find more details about those dependencies in the [Mask documentation](https://github.com/mask-project/mask). Note that that there is no more need to compile Boost from source; like everything else, you can install it now with a MSYS2 package.

4. Install Qt5 (default currently works with Windows 10)
...
Keep MONERO_DIR to 0.2.3.0 and edit seed changes

TODO: Check changes from /monero and /mask for wallet.cpp error
